...
sf = [-e; r1D; -r1D];
rq = [r1D; r1D; r1D];
sq = [r1D[1]*ones(3); r1D[2]*ones(3); r1D[3]*ones(3)];
rf = [r1D; ones(3); r1D; -1*ones(3)];
sf = [-1*ones(3); r1D; ones(3); r1D];
...
# plot([-1;-1].+3/4,[-1;1].+2/4,seriescolor=:black,lw=3,label = nothing)
# plot!([-1;1].+3/4,[1;-1].+2/4,seriescolor=:black,lw=3,label = nothing)
# plot!([-1;1].+3/4,[-1;-1].+2/4,seriescolor=:black,lw=3,label = nothing)
# scatter!(r_tri.+3/4,s_tri.+2/4,markersize=7,markercolor=:royalblue1,markerstrokecolor=:black,markerstrokewidth=2)
# scatter!(rf.+3/4,sf.+2/4,markersize=7,markershape=:rect,markercolor=:darkorange1,markerstrokecolor=:black,markerstrokewidth=2)

plot([-1;-1],[-1;1],seriescolor=:black,lw=3,label = nothing)
plot!([-1;1],[1;1],seriescolor=:black,lw=3,label = nothing)
plot!([1;1],[1;-1],seriescolor=:black,lw=3,label = nothing)
plot!([1;-1],[-1;-1],seriescolor=:black,lw=3,label = nothing)
scatter!(rq,sq,markersize=7,markercolor=:royalblue1,markerstrokecolor=:black,markerstrokewidth=2)
scatter!(rf,sf,markersize=7,markershape=:rect,markercolor=:darkorange1,markerstrokecolor=:black,markerstrokewidth=2)
